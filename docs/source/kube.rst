Scaling [myawesometmlsolution-534a-3f10] With Kubernetes
===========================

Generated On: 2025-02-22 19:24:16 UTC
...
Based on your TML solution [myawesometmlsolution-534a-3f10] - if you want to scale your application with Kubernetes - you will need to apply the following YAML files.

.. list-table::

   * - **YML File**
     - **Description**
   * - :ref:`myawesometmlsolution-534a-3f10.yml`
   ...
   * - :ref:`qdrant.yml`
     - This is the Qdrant deployment YAML.
       ...
       applied if using Step 9 DAG.
   * - :ref:`nginx-ingress-myawesometmlsolution-534a-3f10.yml`
     - If you are scaling your TML solution you must

       apply the nginx-ingressmyawesometmlsolution-534a-3f10.yml; this yaml is 
...
.. code-block:: YAML
   ...
   kubectl apply -f kafka.yml -f secrets.yml -f mysql-storage.yml -f mysql-db-deployment.yml -f myawesometmlsolution-534a-3f10.yml

myawesometmlsolution-534a-3f10.yml
------------------------

.. important::
   Copy and Paste this YAML file: myawesometmlsolution-534a-3f10.yml - and save it locally.
...
.. code-block:: YAML
   ...
   ################# myawesometmlsolution-534a-3f10.yml
     ...
     kind: Deployment
     metadata:
       name: myawesometmlsolution-534a-3f10
     spec:
       selector:
         matchLabels:
           app: myawesometmlsolution-534a-3f10
       ...
       template:
         ...
         spec:
           containers:
           - name: myawesometmlsolution-534a-3f10
             image: maadsdocker/myawesometmlsolution-534a-3f10-amd64:latest
             volumeMounts:
             - name: dockerpath
             ...
             - name: TSS
               value: '0'
             - name: SOLUTIONNAME
               value: 'myawesometmlsolution-534a-3f10'
             - name: SOLUTIONDAG
               value: 'solution_preprocessing_dag-myawesometmlsolution-534a-3f10'
             - name: GITUSERNAME
               value: 'smaurice101'
             - name: GITREPOURL
   ...
   ---
     ...
     kind: Service
     metadata:
       name: myawesometmlsolution-534a-3f10-visualization-service
       labels:
         app: myawesometmlsolution-534a-3f10-visualization-service
     spec:
       type: ClusterIP
       ports:
...
.. code-block::
   ...
   kubectl port-forward deployment/myawesometmlsolution-534a-3f10 34883:34883

After you forward the ports then copy/paste the viusalization URL below and run your dashboard.

.. code-block::

   http://localhost:34883/dashboard.html?topic=iot-preprocess&offset=-1&groupid=&rollbackoffset=400&topictype=prediction&append=0&secure=1
...
.. important::
   ...
   **STEP 4:  Apply nginx-ingress-myawesometmlsolution-534a-3f10.yml to your kubernetes cluster.  First you need to save it locally then apply it:**

nginx-ingress-myawesometmlsolution-534a-3f10.yml
-------------
   ...
   .. code-block::
      ...
      kubectl apply -f nginx-ingress-myawesometmlsolution-534a-3f10.yml
